...
async function walk(directory, root, libraryName, output) {
  let entries;
  try {
  ...
  await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) {
        await walk(fullPath, root, libraryName, output);
        return;
      }
...
export async function scanMedia(libraries) {
  const output = [];
  for (const configuredLibrary of libraries) {
    ...
    await walk(root, root, library.name, output);
  }
